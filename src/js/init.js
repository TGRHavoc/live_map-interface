...
class Initializer {
    ...
    /**
     * @typedef {import("./map").MapWrapper} MapWrapper
     *
     * @param {String} url
        ...
        for (var spriteId in data) {
            if (data.hasOwnProperty(spriteId)) {
                ...
                for (var i in blipArray) {
                    ...
                    var fallbackName =
                        markers.MarkerTypes[spriteId] != undefined &&
                        markers.MarkerTypes[spriteId].hasOwnProperty("name")
                            ? markers.MarkerTypes[spriteId].name
                            : "Unknown Name... Please make sure the sprite exists.";

                    blip.name =
                        blip.hasOwnProperty("name") || blip.name != null
                            ? blip.name
                            : fallbackName;
                    blip.description =
                        blip.hasOwnProperty("description") ||
