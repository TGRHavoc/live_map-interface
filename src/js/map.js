...
class MapWrapper {
    ...
    constructor(socketHandler) {
        ...
        this.markers = window.markers = new Markers(
            Config.getConfig(),
            this.controls
        );
    }
        ...
        for (var spriteId in this.blips) {
            ...
            console.log(
                "Disabled (" +
                    spriteId +
                    ")? " +
                    this.disabledBlips.includes(spriteId)
            );
    ...
    changeServer(nameOfServer) {
        ...
        if (!(nameOfServer in Config.staticConfig.servers)) {
            new Alerter({
                title: Translator.t("errors.server-config.title"),
                text: Translator.t("errors.server-config.message", {
                    nameOfServer,
                }),
            });
            return;
    ...
    mapInit(elementID) {
        ...
        let mapBounds = Utils.getMapBounds(this.Map, this.CurrentLayer);
    ...
    createMarker(draggable, objectRef, title) {
        ...
        if (
            objectRef.data &&
            objectRef.data.isPlayer &&
            Config.getConfig().groupPlayers
        ) {
    ...
    addBlip(blipObj) {
        ...
        if (!blipObj.hasOwnProperty("name")) {
            // Doesn't have a name
            if (
                this.markers.MarkerTypes[spriteId] == null ||
                this.markers.MarkerTypes[spriteId].name == undefined
            ) {
        ...
        if (!blipObj.hasOwnProperty("description")) {
    ...
    updateBlip(blipObj) {
        if (this.doesBlipExist(blipObj)) {
            ...
            if (blipObj.hasOwnProperty("name")) {
                ...
                if (
                    this.markers.MarkerTypes[blipObj.type] != undefined &&
                    this.markers.MarkerTypes[blipObj.type].name != undefined
                ) {
    ...
    playerLeft(playerName) {
        if (
            this.localPlayerCache[playerName] !== undefined &&
            (this.localPlayerCache[playerName].marker != null ||
                this.localPlayerCache[playerName].marker != undefined)
        ) {
    ...
    doPlayerHtmlUpdates(plr) {
        ...
        if (!(plr.identifier in this.localPlayerCache)) {
            this.localPlayerCache[plr.identifier] = {
                marker: null,
                lastHtml: null,
            };
        }
        ...
        const playerMarkerInLocalCache =
            this.localPlayerCache[plr.identifier].marker;
        const playerMarkerFromStore =
            this.MarkerStore[playerMarkerInLocalCache];
        ...
        if (playerMarkerFromStore) {
            ...
            let m = (this.localPlayerCache[plr.identifier].marker =
                this.createMarker(false, obj, plr.name) - 1);
